...
def _parse_findings(report_md: str) -> list[dict]:
    """Extract hypothesis findings from report.md — split on ## headings."""
    findings = []
    sections = re.split(r"\n##\s+", report_md)
    for section in sections[1:]:
        lines = section.strip().split("\n")
        title = lines[0].strip("# ").strip()
        # Skip summary/structural sections — shown separately in the UI
        if title.lower() in ("summary", "executive summary", "data evidence"):
            continue
        body_lines = []
        confidence = "Medium"
        for line in lines[1:]:
            if re.search(r"confidence[:\s]*(high|medium|low)", line, re.IGNORECASE):
                m = re.search(r"(high|medium|low)", line, re.IGNORECASE)
                if m:
                    confidence = m.group(1).capitalize()
            else:
                body_lines.append(line)
        # Preserve full body with line breaks (no truncation)
